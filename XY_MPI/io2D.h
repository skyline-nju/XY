...
public:
  ...
  void read(int i_frame, TFloat* theta_out, int n) const;

  template <typename TFloat>
  void read_last_frame(TFloat* theta_out, int n) const;
...
template<typename TFloat>
void io::Snap_GSD_2::read(int i_frame, TFloat* theta_out, int n) const {
  if (is_root()) {
    uint32_t n_par;
...
template<typename TFloat>
void io::Snap_GSD_2::read_last_frame(TFloat* theta_out, int n) const {
  if (is_root()) {
    int nframes = gsd_get_nframes(handle_);
    if (nframes < 1) {
