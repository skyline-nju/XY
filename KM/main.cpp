...
int main(int argc, char* argv[]) {
...
#else
  char folder[255];
  snprintf(folder, 255, "/home/ps/data/LatticeModel/KM/L%d/", Lx);
#endif
  ...
  // initialize spins
  Ran myran(seed);
  Kuramoto_2<SquareLattice_2> KM_spins(Lx, Ly, T, sigma, h);
  KM_spins.ini(ini_mode, gsd, myran);

  // run
  for (int i = 1; i <= n_step; i++) {
    KM_spins.update(myran);
    log.record(i);
    op.dump(i, KM_spins);
    gsd.dump(i, KM_spins);
  }
