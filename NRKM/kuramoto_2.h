...
public:
  Kuramoto_2(int Lx, int Ly, double T, double sigma, double h,
             double omega0=0., double J0 = 1.0);
  ...
  void ini_omega(TRan& myran, double omega0=0., const std::string& mode = "bimodal");
...
Kuramoto_2<TLattice>::Kuramoto_2(int Lx, int Ly,
                                 ...
                                 double h, double omega0, double J0)
  : XY_2<TLattice>(Lx, Ly, T, h, J0), sigma_(sigma), omega0_(omega0) {
...
template<typename TLattice>
template<typename TRan>
void Kuramoto_2<TLattice>::ini_omega(TRan& myran, double omega0, const std::string& mode) {
  if (mode == "bimodal") {
    size_t n_half = XY_2<TLattice>::N_ / 2;
    for (size_t i = 0; i < n_half; i++) {
      omega_[i] = sigma_ + omega0;
    }
...
void Kuramoto_2<TLattice>::update(TRan& myran) {
  ...
  // update theta
  for (size_t i = 0; i < XY_2<TLattice>::N_; i++) {
    XY_2<TLattice>::update_theta(i, myran, omega_[i]);
